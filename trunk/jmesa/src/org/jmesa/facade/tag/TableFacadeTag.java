...
public class TableFacadeTag extends SimpleTagSupport {
	 ...
	 * 
	 * @param performFilterAndSort
	 *            True if should sort and filter the Collection of Beans (or
	 *            Maps).
	 */
	...
	public Limit getLimit() {
		...
		LimitFactory limitFactory = new LimitFactoryImpl(getId(),
				getWebContext());
	...
	public CoreContext getCoreContext() {
		...
		TagCoreContextFactory coreContextFactory = new TagCoreContextFactory(
				isPerformFilterAndSort(), getWebContext());

		FilterMatcherMap filterMatcherMap = getTableFacadeFilterMatcherMap();
		if (filterMatcherMap != null) {
			Map<MatcherKey, FilterMatcher> filterMatchers = filterMatcherMap
					.getFilterMatchers();
		...
		this.coreContext = coreContextFactory.createCoreContext(getItems(),
				getLimit());
	...
	protected FilterMatcherMap getTableFacadeFilterMatcherMap() {
		...
		try {
			filterMatcherMap = (FilterMatcherMap) Class.forName(
					getFilterMatcherMap()).newInstance();
			SupportUtils.setWebContext(filterMatcherMap, getWebContext());
		} catch (Exception e) {
			logger.error("Could not create the filterMatcherMap ["
					+ getFilterMatcherMap() + "]", e);
		}
	...
	protected Toolbar getToolbar() {
		...
		if (toolbarMaxRowIncrements != null
				&& toolbarMaxRowIncrements.length > 0) {
			toolbarFactory = new ToolbarFactoryImpl(getTable(),
					toolbarMaxRowIncrements, getWebContext(), getCoreContext(),
					exportTypes);
		} else {
			toolbarFactory = new ToolbarFactoryImpl(getTable(),
					getWebContext(), getCoreContext(), exportTypes);
		}
	...
	protected View getTableFacadeView() {
		...
		if (StringUtils.isEmpty(getView())) {
			...
			try {
				...
				logger
						.error("Could not create the view [" + getView() + "]",
								e);
			}
		}
	...
	public HtmlComponentFactory getComponentFactory() {
		...
		this.componentFactory = new HtmlComponentFactory(getWebContext(),
				getCoreContext());
	...
	public void doTag() throws JspException, IOException {
		...
		if (body == null) {
			throw new IllegalStateException(
					"You need to wrap the table in the facade tag.");
		}
		...
		if (tagCoreContext.getPageItems().size() == 0) {
			...
			for (Iterator<Object> iterator = tagCoreContext.getPageItems()
	...
	/**
	 * An abstract to work with the items in a polymorphic manner. Will work
	 * with the regular items until all the rows and columns are processed. Then
	 * will swap out regular items with the new page items. See the
	 * getPageItems() method for more information.
	 */
	protected static class TagCoreContext extends CoreContextImpl {
		private Collection<Object> pageItems;

		public TagCoreContext(Items items, Limit limit,
	...
	/**
	 * An implementation specifically for the tags to work with the
	 * TagCoreContext.
	 * 
	 */
	protected static class TagCoreContextFactory extends CoreContextFactoryImpl {
		public TagCoreContextFactory(boolean performFilterAndSort,
				WebContext webContext) {
			super(performFilterAndSort, webContext);
		}

		@Override
		public CoreContext createCoreContext(Collection<Object> items,
				Limit limit) {
			Items itemsImpl;

			if (isPerformFilterAndSort()) {
				itemsImpl = new ItemsImpl(items, limit, getRowFilter(),
						getColumnSort());
			} else {
				itemsImpl = new ItemsImpl(items, limit, new DefaultRowFilter(),
						new DefaultColumnSort());
			}

			CoreContext coreContextImpl = new TagCoreContext(itemsImpl, limit,
					getPreferences(), getMessages());
