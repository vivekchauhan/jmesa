...
import org.jmesalive.domain.Name;
...
public class PresidentDao {
	...
	private static final Logger log = LoggerFactory.getLogger(PresidentDao.class);
	
    public PresidentDao(WebContext webContext) {
    	this.webContext = webContext;
    }
    
    public Collection<President> getPresidents() {
    	if (webContext.getParameter("restore") != null) {
    		webContext.removeSessionAttribute("presidentCollection");
    	} else {
    		presidents = (List<President>)webContext.getSessionAttribute("presidentCollection");
    	}
        ...
        president.setEmail(getEmail(president.getName().getFirstName(), president.getPoliticalParty()));
        ...
        president.setPoliticalParty("Democratic");
        president.setEmail(getEmail(president.getName().getFirstName(), president.getPoliticalParty()));
        presidents.add(president);
        ...
        webContext.setSessionAttribute("presidentCollection", presidents);
        webContext.setSessionAttribute("lastId", presidents.size());
        
        return presidents;
    }

    public Map<String, President> getPresidentsAsMap() {

        Map<String, President> results = new HashMap<String, President>();

        for (President president : getPresidents()) {
            results.put(String.valueOf(president.getId()), president);
        }

        return results;
    }

    public void savePresident(President p, boolean isNew)
    {
    	if (isNew) {
    ...
    public void deletePresident(President p)
    {
    	presidents.remove(p);
	...
	public static President getNewPresident() {
        President president = new President();
        Name name = new Name("new", "new", "new");
        president.setName(name);
        president.setTerm("00-00");
        president.setBorn(new Date());
        president.setEducation("");
        president.setCareer("new");
        president.setPoliticalParty("new");
        president.setEmail(getEmail(president.getName().getFirstName(), president.getPoliticalParty()));
        
        return president;
	}

    public static String getEmail(String firstName, String politicalParty) {
        if (!StringUtils.isBlank(firstName) && !StringUtils.isBlank(politicalParty)) { 
        	return firstName.split("[\\-,; ]")[0] + "@" + politicalParty.split("[\\-,; ]")[0] + ".com";
        }
