...
public class PresidentDaoImpl extends HibernateDaoSupport implements PresidentDao {
    ...
    public Map getPresidentsByUniqueIds(String property, List uniqueIds) {
        Map result = new HashMap();
        
        String sql = "from President";
        
        String in = "";
        for (Iterator it = uniqueIds.iterator(); it.hasNext();) {
        ...
        List presidents = getHibernateTemplate().find(sql);
        for (Iterator it = presidents.iterator(); it.hasNext();) {
            President president = (President)it.next();
